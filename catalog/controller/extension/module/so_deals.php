		...
		if($setting['display_feature']){
			...
			if(count($setting['product_feature']) > 0){
				...
				foreach($products_deals as $product)
				{
					...
					$description = (($setting['description_maxlength'] != 0 && strlen($product_info['description']) > $setting['description_maxlength'] ) ? html_entity_decode(utf8_substr(strip_tags($product_info['description']), 0, $setting['description_maxlength']), ENT_QUOTES, 'UTF-8') . '..' : html_entity_decode($product_info['description'], ENT_QUOTES, 'UTF-8'));
